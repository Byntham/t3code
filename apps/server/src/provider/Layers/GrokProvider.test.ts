...
it.layer(NodeServices.layer)("checkGrokProviderStatus", (it) => {
  ...
  it.effect("includes filesystem skills for the $ picker when Grok is enabled", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const homeDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3code-grok-skills-snap-" });
      const skillDir = path.join(homeDir, "skills", "tldr");
